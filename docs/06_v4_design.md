...
- 段階4: `SelectedDisplayResolver`(純関数)— TabDesk 前面(改名中)なら直前のフォーカスキャッシュ、
  それ以外は前面アプリの最前面ウィンドウ(CGWindowList) → pid が一致するキャッシュ → マウスの画面 → 主。
  **非同期 AX は使わない**。CGWindowList の補完により未登録窓へのフォーカスや画面間ドラッグ直後も
  マウス位置へ誤フォールバックしない。前面化ルール(切替画面 == 選択画面のときだけ)適用。
...
軽微で許容: 移行バックアップの判定がタブ id の変化のみ(分割を伴わない移行は退避しない)

## 再レビュー(2026-09-02)

- 既に `Tab.displayID` がある v4 データはタブを正として窓側の不整合だけを修復するよう変更。
  欠落した窓の ID で既移行タブが再分割される問題を防止
- 主ディスプレイの役割変更直後に残る旧 active entry を公開 API と UI で無効扱いにし、
  再適用前の新規タブ・登録・移籍が別画面のタブを選ばないようにした
- 選択ディスプレイ解決へ CGWindowList の前面窓位置を追加。未登録窓へのフォーカスと
  画面間ドラッグ直後も、非同期 AX を使わずフォーカス窓の画面を優先する
- 既定タブ名を Core で画面ごとの連番に統一し、URL の tab 省略時も選択画面の active を使う。
  不正な `display` index は別画面へ誤作成せずログ付きで拒否
- 他画面で共有幅を変更したときの contentView 制約と、全画面共通の編集モード表示を同期
- 自由配置窓のサイドバー境界追従は、配置領域の右端が変わらない幅変更時だけ適用。
  解像度・画面配置変更で窓幅を誤変更しない。折りたたみ・画面切断時はリサイズカーソルも明示解除
