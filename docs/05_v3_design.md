...
- ~~既知の非対称: clamp は位置しか動かさないので、狭めた/畳んだとき free の窓は空いた領域へ
  ...
  **サイドバー境界に接していた free の窓(許容 1pt)は右端固定で境界に追従**する
  ...
  縮めた結果が 200pt 未満なら追従せず従来の位置押し戻し)。中ほどの窓は従来どおり clamp のみ。
  エンジンが画面ごとの直前のコンテンツ領域を非永続で覚えて比較する。⇔カーソルは cursorUpdate が
  非アクティブ化パネルに届かなかったため mouseEntered/Exited の push/pop に変更
