...
- ~~既知の非対称: clamp は位置しか動かさないので、狭めた/畳んだとき free の窓は空いた領域へ
  広がらない~~ → 2026-09-02 の実機確認で「狭めたら窓も追従して広がってほしい」との要望を受け変更:
  **サイドバー境界に接していた free の窓(許容 1pt)は右端固定で境界に追従**する
  (`TabEngine.followingSidebarEdge`。狭める/畳むで広がり、広げる/展開で縮む。往復で元に戻る。
  下限なし — 右端を画面内に保つのが優先)。中ほどの窓は従来どおり clamp のみ。
  エンジンが画面ごとの直前のコンテンツ領域を非永続で覚えて比較する。右端も変わる解像度・
  画面配置変更はサイドバー幅変更ではないため追従計算をせず、通常の clamp に任せる。
  さらに **clamp は領域より大きい窓を領域の大きさまで縮める**(タイトルバーのダブルクリックで
  サイドバー分を無視した寸法になった窓や、幅を広げたときに右端が画面外へ出ないように。全 clamp 経路共通)。
  ⇔カーソルは cursorUpdate が非アクティブ化パネルに届かず、非アクティブ時はマウス移動のたびに
  矢印へ戻されるため、mouseEntered/Exited の push/pop + mouseMoved/Dragged での再設定に変更。
  折りたたみ・画面切断ではイベントが届かなくてもカーソルスタックを明示的に片付ける
