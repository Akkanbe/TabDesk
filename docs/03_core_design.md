...
v1 段階 1 で作成し、v2 までの実装を反映した設計。仕様は docs/01_spec.md、実測根拠は docs/02_poc_results.md。
...
- **退避先・配置領域も注入する**(`ScreenLayout`)。本番は接続中の全ディスプレイから都度計算、テストは固定値。
...
(v1 の残件はなし。タブの前後移動ホットキーは 2026-08-26、並べ替え UI とフルスクリーン/最小化除外は
2026-08-28 に実装済み。v2 のタイル・マルチディスプレイ設計は docs/04_v2_design.md)
