...
以下は既定のキー設定です。`Ctrl` は Control（⌃）、`Option` は ⌥ を表します。
ここでいう「タブ」は **TabDesk のサイドバーのタブ**です。ブラウザ内のタブとは別です。
図は操作の流れを示す模式図です。

| やりたいこと | キー／操作 | 対象・結果 |
| --- | --- | --- |
| 次／前のディスプレイへフォーカスを移す | `Ctrl+Option+→` / `Ctrl+Option+←` | 移動先の選択中タブにある窓へ |
| 次／前のタブを表示する | `Ctrl+Tab` / `Ctrl+Shift+Tab` | 選択中ディスプレイのタブを循環 |
| 番号でタブを選ぶ | `Ctrl+Option+1〜9` | 選択中ディスプレイの上から1〜9番目のタブへ |
| 作業中の窓を登録する | `Ctrl+Option+R` | **その窓があるディスプレイ**の選択中タブへ |
| 同じタブ内の別の窓へフォーカスを移す | 目的の窓をクリック | 窓／タイル単位で順送りする専用ホットキーはなし |
| タイルを指定して登録・割り当てする | 「タイルを編集…」で操作 | 専用ホットキーはなし（後述） |
| 自由配置の編集モードを切り替える | `Ctrl+Option+E` | 移動・サイズ変更を記憶するモードを切替 |
| サイドバーを折りたたむ／展開する | `Ctrl+Option+S` | 選択中ディスプレイに作用 |

### 1. フォーカス中のウィンドウを登録する

1. 登録したい窓があるディスプレイで、登録先のタブを選びます。画面切替キーの後にタブ切替キーを使えます。
2. 登録したい窓をクリックして、入力フォーカスを戻します。
3. `Ctrl+Option+R` を押します。サイドバーにその窓が追加され、タイルモードなら最初の空タイルへ配置されます。

```mermaid
flowchart LR
    A["登録先タブを選ぶ<br/>例：画面Bのタブ2"] --> B["登録したい窓をクリック<br/>画面Bにあるエディタ"]
    B -->|"Ctrl+Option+R"| C["画面Bのタブ2に登録"]
    C --> D["タイルモード<br/>最初の空タイルへ配置"]
    C --> E["自由配置<br/>窓の位置・サイズをもとに登録"]
```

**登録先は、画面切替キーで選んだ画面よりも「実際にフォーカス中の窓がある画面」が優先されます。**
画面Bを選択していても、画面Aの窓にフォーカスが残っていれば、登録先は画面Aの選択中タブです。
別画面の窓を登録先の画面へ移して登録するには、登録先サイドバーの「＋ ウィンドウを追加」で窓を選べます。

タブがない画面では登録時にタブを自動作成します。登録済みの窓にもう一度キーを押しても、別タブへの移動や重複登録はしません。
最小化中・macOSのフルスクリーン中の窓は登録できないため、通常表示へ戻してから操作してください。
タイルがすべて埋まっている場合は登録できません。次の「タイルへの割り当て」の手順で空タイルを用意します。

### 2. 目的のウィンドウへフォーカスを移す

**「ディスプレイを選ぶ → タブを選ぶ → 必要なら窓をクリック」**の順で操作します。
たとえば左の画面Aから、右の画面Bのタブ2にあるエディタへ移る場合は次の流れです。

```mermaid
flowchart LR
    A["画面A<br/>現在の作業窓"] -->|"Ctrl+Option+→"| B["画面B<br/>選択中タブの窓へフォーカス"]
    B -->|"Ctrl+Option+2"| C["画面Bのタブ2<br/>最後に使った窓へフォーカス"]
    C --> D{"目的のエディタ？"}
    D -->|"はい"| E["そのまま入力"]
    D -->|"いいえ"| F["表示されたエディタの窓をクリック"]
```

- **ディスプレイの切替**：配置の左→右、同じ横位置なら上→下の順に循環します。`←` は逆順で、1画面だけなら何もしません。ウィンドウ自体を別画面へ移す操作ではなく、操作する画面を選ぶ操作です。マウスポインタは移動しません。
- **タブの切替**：その画面の窓だけが切り替わり、他画面のタブはそのままです。番号はサイドバーの並び順で、存在しない番号は何もしません。`Ctrl+Tab` は末尾から先頭へ、`Ctrl+Shift+Tab` は先頭から末尾へ循環します。
- **フォーカスされる窓**：画面・タブを切り替えると、そのタブで最後に使った操作可能な登録窓へ戻ります。その窓を使えない場合は、別の操作可能な登録窓を順に試します。同じタブの番号をもう一度押した場合も、フォーカスを戻します。
- **目的の窓が別のタイルにある場合**：タブを表示した後、その窓自体をクリックします。`Ctrl+Option+1〜9` の番号はタブ番号であり、タイル番号ではありません。

空タブなどでフォーカスできる窓がなくても、画面の選択は維持されます。サイドバーの対象画面を示す枠線がオレンジ色になったら、入力先へフォーカスできていない状態です。そのままタブ切替キーで別のタブを選べます。
クリックや `Cmd+Tab` で別画面の窓へフォーカスを移すと、操作対象の画面も追従します。
「フォーカスで自動切替」がON（既定）の場合、非選択タブの登録窓へフォーカスが移ると、そのタブも表示されます。

### 3. タイルへの割り当てを決める・変更する

新しいタブは **1枚のタイル**から始まります。2つ以上の窓を並べる場合は、先にタブを右クリック →「タイルを編集…」を開き、タイルを選んで「左右に分割」または「上下に分割」→「適用」を押します。
分割や割り当ての変更は「適用」まで実際の窓へ反映されません。

```mermaid
flowchart TB
    A["新しいタブ：タイル1枚"] -->|"左右に分割 → 適用"| B["タイル1：空　｜　タイル2：空"]
    B -->|"ブラウザをフォーカス → Ctrl+Option+R"| C["タイル1：ブラウザ　｜　タイル2：空"]
    C -->|"エディタをフォーカス → Ctrl+Option+R"| D["タイル1：ブラウザ　｜　タイル2：エディタ"]
    D -->|"編集画面でタイル1を選択<br/>割り当てでエディタを選択 → 適用"| E["タイル1：エディタ　｜　タイル2：ブラウザ"]
```

| 登録・配置の方法 | 手順 |
| --- | --- |
| 空いている場所へ順に登録 | 登録する窓をフォーカスして `Ctrl+Option+R`。タイル編集画面の番号順で最初の空タイルを使います。編集画面で選択中のタイルを指定するキーではありません。 |
| 空タイルを指定して新しく登録 | 編集内容を「適用」→目的の空タイルを選択→「このタイルにウィンドウを追加…」で窓を選択。 |
| 登録済みの窓を別タイルへ移す | 移動先タイルを選択→「選択したタイルへの割り当て：」のプルダウンで窓を選択→「適用」。移動先に別の窓があれば、2つの窓の割り当てを入れ替えます。 |

`Ctrl+Option+E` の編集モードは自由配置用です。タイル配置では、窓をドラッグしても固定位置へ戻るため、上記の編集画面で配置を変更してください。
ウィンドウの登録を解除しても空タイルは残ります。配置できない警告が出た場合は、タイルを広げるか割り当てを変えて「適用」します。
詳しくは[タイルモード](#タイルモード)を参照してください。

### ホットキーが他アプリと競合するとき

TabDeskのホットキーはグローバルです。起動中は `Ctrl+Tab` がブラウザなどへ届かなくなります。
メニューバーの「ホットキー設定を開く」で該当欄をクリックして別の組み合わせを押し、「保存して適用」してください。
各行の「×」で割り当てを解除することもできます。変更後は、このページの既定キーを自分の設定に読み替えてください。
